...
description: |
  Welcome to my new blog, casualR. I hope you will find some useful content !
author:
...
To extract the information on the fonts inside the pdf we use Poppler, the [PDF rendering library](https://poppler.freedesktop.org/) and its cpp API. 
...
The role of the function annotate_txt_pdf() is to load the required model and use the library udpipe to tokenize and annotate the text. Please refer to the vignette or the [excellent website](https://bnosac.github.io/udpipe/en/) of udipipe to get more details on the Conll-U format.
The reason behind this annotation is that we will need this information to estimate where the most likely is the section title. For example, if it is the first word of a sentence, if the the word of the sentence is also a section title, etc.
...
The other information we use, and the reason why we work directly on a pdf instead of a text, is the fonts information from the pdf, the font and the fontsize of the words inside the pdf. To do this we use poppler, a [PDF rendering library](https://poppler.freedesktop.org/) and the cpp API of poppler. We extract this information using a specific version of pdftools, reason why the package need a version of poppler > 0.89 as well as a recent version of pdftools. 
...
Knowing the relative order of the sections from one side, their names and the information from the Conll-U dataframe (position inside the sentence, or the other words in the sentence) we can estimate the position of the different sections inside the Conll-U dataframe.
...
More seriously, the extraction will fail on article of one or two pages, because of a function that remove sections names that have the same number of occurences than the numbers of pages, since some journals have names including "Materials", or "Results".

If you tried this package for your project, by curiosity and have some comments or question, or if it does not work on your favorite article, feel free to open an issue on [the github repository of the projet](https://github.com/ec-jrc/jrc_f2_refine), I would be glad to take a look it.
